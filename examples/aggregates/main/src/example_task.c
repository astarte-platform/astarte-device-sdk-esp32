...
static void astarte_data_events_handler(astarte_device_data_event_t *event)
{
    ...
    if ((strcmp(event->interface_name, server_datastream_interface.name) == 0)
        ...
        /*  event->bson_value shoul be containing:
         *  0x4F 0x00 0x00 0x00 {
        ...
        void *booleanarray = astarte_bson_key_lookup(
        ...
        if ((booleanarray != NULL) && (booleanarray_type == BSON_TYPE_ARRAY)
            ...
            void *boolean = astarte_bson_first_item(booleanarray);
            if (boolean != NULL) {
                rx_data.booleans[0] = *(uint8_t *) (boolean + 3);

                for (uint8_t i = 1; i < 4; i++) {
                    boolean = astarte_bson_next_item(booleanarray, boolean);
                    if (boolean != NULL) {
                        rx_data.booleans[i] = *(uint8_t *) (boolean + 3);
                    } else {
                        parsing_error = 1u;
                        break;
                    }
        ...
        void *longinteger = astarte_bson_key_lookup(
    ...
    if (parsing_error == 0u) {
        ...
        // ESP_LOGI(TAG, "Document size %d.", doc_len);
        ...
        ESP_LOGI(TAG, "Device aggregate sent, using device_id: 24.");
    } else {
