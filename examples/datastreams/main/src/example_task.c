...
//         && strcmp(event->path, "/question") == 0 && event->bson_element.type ==
//         BSON_TYPE_BOOLEAN) { bool invert_reply = *((bool *) event->user_data); bool answer =
//         invert_reply ^ astarte_bson_deserializer_element_to_bool(event->bson_element);
