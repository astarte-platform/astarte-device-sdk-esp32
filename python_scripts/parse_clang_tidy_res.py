...
python3 -m pylint --rcfile=./python_scripts/.pylintrc ./python_scripts/*.py
...
def warn_txt_parse(
    ...
    with open(warn_txt_path, encoding="utf-8") as warn_txt_fp:
        ...
        for warn_txt_line in warn_txt_lines:
            ...
            if warn_header_m:
                ...
                if skip_clang_diag and clang_comp_err_pattern.match(warn_header.code):
                    skip_warning = True
                    comp_warn_count += 1
                elif re.match(code_filter, warn_header.code) and re.match(
                    file_filter, warn_header.path
                ):
                    skip_warning = False
                    good_warn_count += 1
        ...
        if skip_clang_diag:
            print(stylize(f"\nRemoved {comp_warn_count} compiler errors.", colored.fg(WARNING)))

        print(stylize(f"\nFound {good_warn_count} errors/warnings/notes.\n", colored.fg(INFO)))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="file_filter",
        default=r".*",
        help="A regex used for filtering the files.",
    )
    parser.add_argument(
        "-t",
        "--trim",
    ...
    if warn_txt_parse(
        warnings_txt_path, args.trim, args.code_filter, args.file_filter, args.remove_clang_diag
    ):
        sys.exit(1)
