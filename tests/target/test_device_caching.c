...
void test_device_caching_synchronization_set_get(void)
{
    ...
    // When not set
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_NOT_FOUND, device_caching_synchronization_get(&sync));

    // Set true
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK, device_caching_synchronization_set(true));

    // Read true
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK, device_caching_synchronization_get(&sync));
    TEST_ASSERT_EQUAL(true, sync);

    // Set false
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK, device_caching_synchronization_set(false));
...
void test_device_caching_introspection_set_get(void)
{
    const char introspection[]
        = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque diam metus, viverra ac "
          ...
          "lorem augue. Sed varius dolor a orci condimentum porta. Ut accumsan finibus tortoreget "
    ...
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_DEVICE_CACHING_OUTDATED_INTROSPECTION,
        device_caching_introspection_check("some introspection", sizeof("some introspection")));

    // Store the introspection
    TEST_ASSERT_EQUAL(
        ASTARTE_RESULT_OK, device_caching_introspection_set(introspection, sizeof(introspection)));
    ...
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_introspection_check(introspection, sizeof(introspection)));

    // Store the introspection
    const char updated_introspection[] = "Very simple introspection";
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_introspection_set(updated_introspection, sizeof(updated_introspection)));
    ...
    // Check correct introspection
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_introspection_check(updated_introspection, sizeof(updated_introspection)));
}
...
void test_device_caching_property_store_load_delete_cycle(void)
{
    ...
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_NOT_FOUND,
        device_caching_property_delete(interface1.name, interface1.mappings[0].endpoint));

    // Load does not work when empty storage
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_NOT_FOUND,
        device_caching_property_load(interface1.name, interface1.mappings[0].endpoint, NULL, NULL));
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_NOT_FOUND,
        device_caching_property_load(interface1.name, interface1.mappings[1].endpoint, NULL, NULL));
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_NOT_FOUND,
        device_caching_property_load(interface2.name, interface2.mappings[0].endpoint, NULL, NULL));
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_NOT_FOUND,
        device_caching_property_load(interface2.name, interface2.mappings[1].endpoint, NULL, NULL));

    // Store properties
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_store(interface1.name, interface1.mappings[0].endpoint,
            interface1.major_version, astarte_data_from_string(I1_P1_PAYLOAD)));
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_store(interface1.name, interface1.mappings[1].endpoint,
            interface1.major_version, astarte_data_from_string(I1_P2_PAYLOAD)));
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_store(interface2.name, interface2.mappings[0].endpoint,
            interface2.major_version, astarte_data_from_string(I2_P1_PAYLOAD)));

    // Load properties
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_load(
            interface1.name, interface1.mappings[0].endpoint, &i1_major_read, &i1_p1_data_read));
    ...
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_load(
            interface1.name, interface1.mappings[1].endpoint, &i1_major_read, &i1_p2_data_read));
    TEST_ASSERT_EQUAL(interface1.major_version, i1_major_read);
    TEST_ASSERT_EQUAL(
    ...
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_load(
            interface2.name, interface2.mappings[0].endpoint, &i2_major_read, &i2_p1_data_read));
    ...
    // Delete property
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_delete(interface1.name, interface1.mappings[1].endpoint));
    ...
    // Delete property
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_delete(interface2.name, interface2.mappings[0].endpoint));
...
void test_device_caching_property_get_device_properties_string(void)
{
    ...
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_get_device_properties_string(
            &introspection, NULL, &read_string_size));
    ...
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_store(interface2.name, interface2.mappings[0].endpoint,
            interface2.major_version, astarte_data_from_string(I2_P1_PAYLOAD)));
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_store(interface2.name, interface2.mappings[1].endpoint,
            interface2.major_version, astarte_data_from_string(I2_P2_PAYLOAD)));
    ...
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_get_device_properties_string(
            &introspection, read_string, &read_string_size));
    ...
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_delete(interface2.name, interface2.mappings[0].endpoint));
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK,
        device_caching_property_delete(interface2.name, interface2.mappings[1].endpoint));
...
void test_device_caching_property_iteration(void)
{
    ...
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_OK, device_caching_property_iterator_init(&iterator));
    ...
    device_caching_property_iterator_terminate(&iterator);
}
...
void test_device_caching_property_iteration_empty_memory(void)
{
    ...
    // Create iterator
    device_caching_property_iterator_t iterator;
    TEST_ASSERT_EQUAL(ASTARTE_RESULT_NOT_FOUND, device_caching_property_iterator_init(&iterator));
